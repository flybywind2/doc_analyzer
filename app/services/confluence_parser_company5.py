"""
Confluence Parser Service (Company Version 5 with Emoji Detection from IMG tags)
Supports: Tables, Headings (h1~h6), Bold, Italic, Indentation (spaces/tabs), Emoji conversion
Detects Confluence emoji SVG images and converts them to Unicode emojis
"""
import re
import time
import json
...
# Confluence emoji name to Unicode mapping
EMOJI_MAP = {
    # Smileys
    "smile": "😊",
    "sad": "😢",
...
def get_emoji_from_name(name: str) -> Optional[str]:
    """
    Convert Confluence emoji name to Unicode emoji

    Args:
        name: Emoji name (e.g., "light bulb", "thumbs up")

    Returns:
        Unicode emoji character or None if not found
    """
    if not name:
        return None
    ...
    if normalized_hyphen in EMOJI_MAP:
        return EMOJI_MAP[normalized_hyphen]

    # Try with spaces replaced by underscores
    normalized_underscore = normalized.replace(' ', '_')
    if normalized_underscore in EMOJI_MAP:
        return EMOJI_MAP[normalized_underscore]
...
def html_to_text(element, base_url: str = "", auth: Optional[tuple] = None) -> str:
    ...
    - <ac:emoticon> → Unicode emoji character (e.g., 💡)
    ...
    def process_element(elem, preserve_whitespace=False):
        ...
        if elem.name is None:
            ...
            # Handle images - check if it's an emoji or real image
            src = elem.get('src', '')
            alt = elem.get('alt', 'image')

            # Check if this is a Confluence emoji (SVG in emoticons path)
            is_emoji = (
                'emoticons' in src.lower() or
                (src.endswith('.svg') and alt and alt.lower() != 'image')
            )

            if is_emoji:
                # Try to convert emoji name to Unicode
                emoji_char = get_emoji_from_name(alt)
                if emoji_char:
                    result.append(emoji_char)
                else:
                    # Fallback to emoji name format if not in map
                    result.append(f':{alt}:')
            else:
                # Real image - download and save locally
                ...
                if src:
                    ...
                    if img_url and auth:
                        ...
                        result.append(f'\n![{alt}]({img_url})\n')
        elif elem.name == 'ac:emoticon':
            # Handle Confluence emoticons - convert to Unicode emoji
            emoji_id = elem.get('ac:emoji-id', '')
            emoji_name = elem.get('ac:name', '')
            if emoji_id:
                try:
                    # Convert hex string to Unicode character
                    # e.g., "1f4a1" → 💡
                    emoji_char = chr(int(emoji_id, 16))
                    result.append(emoji_char)
                except (ValueError, OverflowError):
                    # Fallback to emoji name if conversion fails
                    result.append(f':{emoji_name}:' if emoji_name else '�')
            elif emoji_name:
                # Fallback to emoji name format

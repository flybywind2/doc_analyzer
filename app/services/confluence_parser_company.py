"""
Confluence Parser Service (Company Version with Markdown Table Support)
"""
import re
import time
import json
...
def html_table_to_markdown(html_content, base_url: str = "", auth: Optional[tuple] = None) -> str:
    """
    Convert HTML content (including tables) to markdown format

    Args:
        html_content: HTML string or BeautifulSoup element (may contain tables)
        base_url: Base URL for resolving relative image URLs
        auth: Confluence authentication tuple for downloading images

    Returns:
        Markdown formatted string with tables converted to markdown tables
    """
    if not html_content:
        return ""
    ...
    # Now use html_to_text for the rest (images, text, etc.)
                        ...
                        if header_row:
                            ...
                            if next_row:
                                ...
                                if content_row:
                                    ...
                                    if content_cell:
                                        # HTML을 마크다운으로 변환 (테이블 포함, 이미지 다운로드)
